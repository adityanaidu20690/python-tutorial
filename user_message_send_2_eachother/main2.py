...
def dashboard(user_id):
    if request.method == 'POST':
        ...
        if user_id == 'user1':
            ...
            return redirect(url_for('dashboard', user_id='user1'))
        elif user_id == 'user2':
            reply = request.form['message']
            users['user1']['message'] = reply
            flash(f"Message sent to user1: {reply}")
            return redirect(url_for('dashboard', user_id='user2'))

    else:
        if user_id == 'user1':
            return render_template('user1_dashboard.html', user_id=user_id)
        elif user_id == 'user2':
            user2_message = users['user2']['message']
            return render_template('user2_dashboard.html', message=user2_message, user_id=user_id)
